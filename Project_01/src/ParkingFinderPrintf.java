//**********************************************************************************************************************************************
// I have updated this program to (nearly) only use printf commands. It is not significantly different from the original. Pay that little mind.
//**********************************************************************************************************************************************
...
public class ParkingFinderPrintf {
    public static void main(String[] args) {
        ...
        try {
            ...
            // 2. b.
            System.out.printf("%nThe position of your vehicle is:\tX: %1d   Y: %1d%n%n", carX, carY);
            ...
            ParkingSpot spot2 = new ParkingSpot("Pennsylvania Avenue", rng.nextInt(100), rng.nextInt(100));    //ParkingSpot spot1 = new ParkingSpot("2nd Street", rng.nextInt(100), rng.nextInt(100));
            ParkingSpot spot3 = new ParkingSpot("Wallaby Way",         rng.nextInt(100), rng.nextInt(100));    //ParkingSpot spot1 = new ParkingSpot("3rd Street", rng.nextInt(100), rng.nextInt(100));
            ParkingSpot spot4 = new ParkingSpot("Negro Arroyo Lane",   rng.nextInt(100), rng.nextInt(100));    //ParkingSpot spot1 = new ParkingSpot("4th Street", rng.nextInt(100), rng.nextInt(100));
            ...
            // 4. a, b, & c
            System.out.printf(java.util.Locale.US,"Spot 1: %80s%n\tDistance:   %2d%n\tTotal Cost:   $%,.2f%n"  , spot1, (Math.abs(spot1.getLocationX() - carX) + Math.abs(spot1.getLocationY() - carY)), spot1.getCostPerInterval() * Math.ceil((double)parkingTime / 10));
            System.out.printf(java.util.Locale.US,"Spot 2: %80s%n\tDistance:   %2d%n\tTotal Cost:   $%,.2f%n"  , spot2, (Math.abs(spot2.getLocationX() - carX) + Math.abs(spot2.getLocationY() - carY)), spot2.getCostPerInterval() * Math.ceil((double)parkingTime / 10));
            System.out.printf(java.util.Locale.US,"Spot 3: %80s%n\tDistance:   %2d%n\tTotal Cost:   $%,.2f%n"  , spot3, (Math.abs(spot3.getLocationX() - carX) + Math.abs(spot3.getLocationY() - carY)), spot3.getCostPerInterval() * Math.ceil((double)parkingTime / 10));
            System.out.printf(java.util.Locale.US,"Spot 4: %80s%n\tDistance:   %2d%n\tTotal Cost:   $%,.2f%n%n", spot4, (Math.abs(spot4.getLocationX() - carX) + Math.abs(spot4.getLocationY() - carY)), spot4.getCostPerInterval() * Math.ceil((double)parkingTime / 10));
            ...
            if      (closestSpotDistance == Math.abs((spot1.getLocationX() - carX)) + Math.abs((spot1.getLocationY() - carY)))
                System.out.printf("Distance to closest spot: %2d%nClosest spot: %80s%n", closestSpotDistance, spot1);

            else if (closestSpotDistance == Math.abs((spot2.getLocationX() - carX)) + Math.abs((spot2.getLocationY() - carY)))
                System.out.printf("Distance to closest spot: %2d%nClosest spot: %80s%n", closestSpotDistance, spot2);

            else if (closestSpotDistance == Math.abs((spot3.getLocationX() - carX)) + Math.abs((spot3.getLocationY() - carY)))
                System.out.printf("Distance to closest spot: %2d%nClosest spot: %80s%n", closestSpotDistance, spot3);

            else if (closestSpotDistance == Math.abs((spot4.getLocationX() - carX)) + Math.abs((spot4.getLocationY() - carY))) 
                System.out.printf("Distance to closest spot: %2d%nClosest spot: %80s%n", closestSpotDistance, spot4);
            ...
            // "real world" so I'm going to skip it. I don't want my project to be (more of) a pain to read. I'm probably the only person that would notice Dr. Sevigny's toString formats for 
            // currency anyways. 
            //
